...
fun BudgetCard(
    ...
    val noBudget = budgetState.budget == 0.0
    val barColor = when {
    ...
    Card(
        ...
        Column(
            ...
            if (noBudget) {
                ...
                val remaining = budgetState.budget - budgetState.spent
                Row(
                    modifier = Modifier.fillMaxWidth(),
                    horizontalArrangement = Arrangement.SpaceBetween,
                    verticalAlignment = Alignment.CenterVertically
                    ...
                    Text(
                        text = if (budgetState.isOverBudget)
                            "-${formatDollar(-remaining, currencySymbol)} over"
                        else
                            "${formatDollar(remaining, currencySymbol)} left",
                        style = MaterialTheme.typography.bodySmall,
                        fontWeight = FontWeight.SemiBold,
                        color = if (budgetState.isOverBudget) ErrorRed else MaterialTheme.colorScheme.onSurface
                    )
                    Text(
...
private fun BudgetCardPreview() {
    ExpenseTrackerTheme {
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            BudgetCard(BudgetState(budget = 0.0, spent = 0.0, alertThreshold = 80), "$")
            BudgetCard(BudgetState(budget = 2000.0, spent = 1240.0, alertThreshold = 80), "$")
            BudgetCard(BudgetState(budget = 2000.0, spent = 1750.0, alertThreshold = 80), "$")
            BudgetCard(BudgetState(budget = 2000.0, spent = 2240.0, alertThreshold = 80), "$")
        }
    }
}
